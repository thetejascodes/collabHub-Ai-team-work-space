...
- **Comments & Discussions** - Add comments to tasks with threading, attachments, mentions, pagination, and sorting
- **Activity Tracking** - Monitor all workspace activities and changes
- **Notifications** - Per-user notification records for new comments and mentions
...
| Method | Endpoint | Description |
|--------|----------|-------------|
| GET | `/tasks/:taskId/comments` | Get task comments with pagination, sorting, and optional parentCommentId thread filter |
| POST | `/tasks/:taskId/comments` | Add comment or reply to a parent comment |
| GET | `/tasks/:taskId/comments/:commentId` | Get comment details and replies |
| DELETE | `/tasks/:taskId/comments/:commentId` | Delete comment and its direct replies |
...
```typescript
{
  ...
  parentCommentId?: ObjectId (Comment),
  mentions?: ObjectId[],
  attachments?: {
    filename: string,
    url: string,
